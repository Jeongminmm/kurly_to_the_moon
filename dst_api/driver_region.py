...
import numpy as np
region_driver = [[0 for col in range(10)] for row in range(11)]
#print(region_driver)
region_weight = [[0 for col in range(10)] for row in range(11)]
region_x = 10
region_y = 11
weight=list()
...
def getRegion_test(max_weight):
    tst_regionList = np.random.rand(11,10) * max_weight
...
def findclose_region(i,j,region_x,region_y):
    ...
    elif ((j+1)>=region_y and region_driver[i][j-1]!=0):
        return region_driver[i][j-1]
...
def change_region(before_driver,new_driver,target_x,target_y):
...
if __name__ == '__main__':
    ...
    driver=4
    midRegion = setMidRegion(4,tst_regionList.shape)
